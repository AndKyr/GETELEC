...
class Barrier(Tabulator):
    ...
    def Interpolate_Gammow(self):
        ...
        data = self.Gammow_Exponent_for_Parameters(self._field, self._radius, self._gamma)
...
class Metal_Emitter(Emitter):
    ...
    def _Integration_Limits(self):
        ...
        resolution = NGi #128
        self._maxbeta = np.polyval(self.barrier._gammow_derivative_coefficients, min(self.workfunction, self.barrier._energy_bottom_barrier))
        ...
        elif (self.barrier._gammow_derivative_top_barrier * self.kT > .95):
            workfunction_center = self.workfunction - self.barrier._energy_top_barrier
            energy_top = workfunction_center + 10 * self.kT
            energy_bottom = workfunction_center - 10 / self.barrier._gammow_derivative_top_barrier
            return np.linspace(energy_bottom, energy_top, resolution)
        
        else:
            rootpoly = np.copy(self.barrier._gammow_derivative_coefficients)
    ...
    def Current_Density(self):
        ...
        args = tuple([self.workfunction] + [self.kT] + [self.barrier._energy_top_barrier] + [self.barrier._energy_bottom_barrier] + [self.barrier._gammow_derivative_top_barrier] + [self.barrier._gammow_derivative_bottom_barrier] + list(self.barrier._gammow_coefficients))
        try:
    ...
    def Current_Density_educational(self, plot = False):
        ...
        if(plot):
            ...
            print ("Wmin, Wmax, Work = ", self.barrier._energy_bottom_barrier, self.barrier._energy_top_barrier, self.workfunction)
            print("maxbeta, minbeta, dGmax, beta_T: ", self._maxbeta, self.barrier._gammow_derivative_bottom_barrier, self.barrier._gammow_derivative_top_barrier, 1/ self.kT)
...
class Semiconductor_Emitter(Emitter):
    ...
    def _Integration_Limits_for_Semiconductors(self):
        ...
        self._Evlow = min(self._Evhigh, 0) - 20 / self.barrier._gammow_derivative_bottom_barrier
...
def current_metal_emitter(Field:array, Radius:array, Gamma:array, Workfunction:array, Temperature:array):
    ...
    for i in range(len(Field)):
        ...
        metal_emitter.barrier.Interpolate_Gammow()
...
def current_semiconductor_emitter(Field:array, Radius:array, Gamma:array ,Ec:array, Ef:array, Eg:array, Temperature:array):
    ...
    for i in range(len(Field)):
        ...
        semiconductor_emitter.barrier.Interpolate_Gammow()
