...
!Calculates current density, main module function
    ...
    if (this%R < 0.2d0) then
        this%R = 0.2d0
        if (debug .and. verbose) print *, 'Warning: R<0.2 inputed'
    endif
...
!Calculates Gamow exponent in the general case: Choose appropriate approximation
    ...
    if (x>0.4d0) then !the second root is far away
        this%xmax = min(this%W * this%gamma / this%F, 10.d0)!maximum length 10
...
subroutine gamow_KX(this) 
    ...
    temp =  this%F **1.5d0 / sqrt(Q) - 4* this%F / this%R
    temp  =  max(temp,1.d-2)  !make sure that sqrt is positive, avoid NaNs
    this%minbeta = 16.093d0/sqrt(temp)
