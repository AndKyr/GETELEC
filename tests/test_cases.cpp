...
TEST(BandEmitterTest, CurrentDensityMethodComparison2){
    ModifiedSNBarrier barrier;
    TransmissionSolver solver(&barrier, Config().transmissionSolverParams, 10., 1);
    BandEmitter emitter(solver);
    mt19937 generator(1987);
    ...
    for (int i = 0; i < 64; i++){
        ...
        for (size_t j = 0; j < totalEnergies.size(); j++){
            EXPECT_NEAR(totalEnergyDistributions[j], emitter.totalEnergyDistributionIntegrateParallel(totalEnergies[j]) * CONSTANTS.SommerfeldConstant, 1.e1 * emitter.getToleranceForValue(totalEnergyDistributions[j]));
        }
    }
}
