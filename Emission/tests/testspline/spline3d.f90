...
integer, parameter		:: kx=4, ky=4, kz=4, iknot=0, nline=64
...
print *, 'phi file read successfully. First row is:'
print *, phi(1,1,:)
...
jstart=ny/2
do kstart=1,nz
	if (phi(nx/2,ny/2,k)>1.d-8) exit
enddo

rstart=[x(istart),y(jstart),kstart]
...
call db3ink(x,nx,y,ny,z,nz,fcn,kx,ky,kz,iknot,tx,ty,tz,fcn,iflag)
